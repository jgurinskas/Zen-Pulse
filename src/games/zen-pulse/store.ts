...
const KEY_AUTO_CALM = `${PREFIX}autoCalm`;
...
export function loadAutoCalmApplied(): boolean {
  if (typeof window === 'undefined') return false;
  return window.localStorage.getItem(KEY_AUTO_CALM) === '1';
}

export function saveAutoCalmApplied(applied: boolean) {
  if (typeof window === 'undefined') return;
  try {
    if (applied) {
      window.localStorage.setItem(KEY_AUTO_CALM, '1');
    } else {
      window.localStorage.removeItem(KEY_AUTO_CALM);
    }
  } catch {
...
export function resetAutoCalmApplied() {
  saveAutoCalmApplied(false);
}
